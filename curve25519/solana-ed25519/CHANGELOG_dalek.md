...
* Backend override is now via cfg(curve25519_backend) over additive features
* Provide override to select `u32` or `u64` backend via cfg(curve25519_bits)
...
* Require including a new trait, `use curve25519::traits::BasepointTable`
...
* Add support for using the `zeroize` traits with all point types.
  ...
  consumers of `curve25519` should call these methods manually
  when needed.
