...
// We need the const 2^((p+3)/8) for elligator_encode. This def is checked in tests::consts()
#[cfg(feature = "digest")]
    ...
    fn basepoint_montgomery_to_edwards() {
        ...
        assert_eq!(
            ...
            constants::X25519_BASEPOINT
                .to_edwards(0)
                .expect("sign bit 0 should decompress")
        );
        ...
        assert_eq!(
            ...
            constants::X25519_BASEPOINT
                .to_edwards(1)
                .expect("sign bit 1 should decompress")
        );
    }
